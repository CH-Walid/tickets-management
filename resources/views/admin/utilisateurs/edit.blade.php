...
<div class="max-w-2xl mx-auto p-8 bg-white/80 backdrop-blur rounded-3xl shadow-2xl space-y-8 animate-fade-in-up">
    <form action="{{ route('admin.utilisateurs.update', $utilisateur->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
        @csrf
        @method('PUT')
        <div class="flex flex-col items-center gap-4 mb-8">
            <div class="relative group">
                @php $photo = $utilisateur->photo ?? null; @endphp
                <div id="avatar-preview-container">
                    <x-user-avatar :user="$utilisateur" size="24" font="2xl" class="w-24 h-24 rounded-full object-cover border-4 border-blue-200 shadow cursor-pointer transition-transform group-hover:scale-105" id="profile-photo-preview" :cacheBuster="time()" />
                </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="text" name="nom" value="{{ old('nom', $utilisateur->nom) }}" class="border border-gray-200 rounded-lg px-3 py-2 w-full focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
            </div>
            <div>
                <label class="block mb-1 text-sm font-medium text-gray-700">Prénom</label>
                <input type="text" name="prenom" value="{{ old('prenom', $utilisateur->prenom) }}" class="border border-gray-200 rounded-lg px-3 py-2 w-full focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
            </div>
            <div class="md:col-span-2">
                <label class="block mb-1 text-sm font-medium text-gray-700">Email</label>
                <input type="email" name="email" value="{{ old('email', $utilisateur->email) }}" class="border border-gray-200 rounded-lg px-3 py-2 w-full focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
            </div>
            <div class="md:col-span-2">
                <label class="block mb-1 text-sm font-medium text-gray-700">Téléphone</label>
                <input type="text" name="phone" value="{{ old('phone', $utilisateur->phone) }}" class="border border-gray-200 rounded-lg px-3 py-2 w-full focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
            </div>
            <div class="md:col-span-2">
            ...
            <div class="md:col-span-2">
                ...
                <select name="service_id" class="border border-gray-200 rounded-lg px-3 py-2 w-full focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    ...
                    @foreach($services as $service)
                        <option value="{{ $service->id }}" @if(old('service_id', $utilisateur->userSimple->service_id ?? null) == $service->id) selected @endif>{{ $service->titre }}</option>
                    @endforeach
                </select>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if(deleteBtn && avatarContainer && deleteHidden) {
        deleteBtn.addEventListener('click', function(e) {
            e.preventDefault();
            avatarContainer.innerHTML = `@php echo str_replace("'", "\'", view('components.user-avatar', ['user' => $utilisateur, 'size' => 24, 'font' => '2xl', 'class' => 'w-24 h-24 rounded-full object-cover border-4 border-blue-200 shadow cursor-pointer transition-transform group-hover:scale-105', 'id' => 'profile-photo-preview'])->render()); @endphp`;
