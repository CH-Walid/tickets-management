    ...
    <form action="{{ route('admin.profil.update') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="text" name="nom" value="{{ old('nom', Auth::user()->nom) }}" class="border border-gray-200 rounded-lg px-3 py-2 w-full focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
            </div>
            <div>
                <label class="block mb-1 text-sm font-medium text-gray-700">Prénom</label>
                <input type="text" name="prenom" value="{{ old('prenom', Auth::user()->prenom) }}" class="border border-gray-200 rounded-lg px-3 py-2 w-full focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
            </div>
            <div class="md:col-span-2">
                <label class="block mb-1 text-sm font-medium text-gray-700">Email</label>
                <input type="email" name="email" value="{{ old('email', Auth::user()->email) }}" class="border border-gray-200 rounded-lg px-3 py-2 w-full focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
            </div>
            <div class="md:col-span-2">
                <label class="block mb-1 text-sm font-medium text-gray-700">Téléphone</label>
                <input type="text" name="phone" value="{{ old('phone', Auth::user()->phone) }}" class="border border-gray-200 rounded-lg px-3 py-2 w-full focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
            </div>
        ...
        <div class="flex gap-2 mt-6 justify-end">
            ...
            <a href="{{ route('admin.profil') }}" class="px-6 py-2 bg-gray-200 text-gray-700 rounded-lg shadow hover:bg-gray-300 transition font-semibold">Annuler</a>
        </div>
    </form>
</div>
<style>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if(deleteBtn && avatarContainer && deleteHidden) {
        deleteBtn.addEventListener('click', function(e) {
            e.preventDefault();
            avatarContainer.innerHTML = `@php echo str_replace("'", "\'", view('components.user-avatar', ['user' => Auth::user(), 'size' => 24, 'font' => '2xl', 'class' => 'w-24 h-24 rounded-full object-cover border-4 border-blue-200 shadow cursor-pointer transition-transform group-hover:scale-105', 'id' => 'profile-photo-preview'])->render()); @endphp`;
