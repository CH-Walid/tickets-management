        ...
        >
            <div class="p-8">
                <form
                    id="incidentForm"
                    method="POST"
                    action="{{ route('chef.tickets.update', $ticket->id) }}"
                ...
                >
                    ...
                    <!-- Statut -->
                <div>
                    <label for="status" class="block text-gray-700 dark:text-gray-300 font-medium mb-1">Statut *</label>
                    <select
                        id="status"
                        name="status"
                        required
                        class="w-full bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg p-3 appearance-none focus:outline-none focus:ring-2 focus:ring-blue-500"
                    >
                        <option value="">-- Sélectionner le statut --</option>
                        <option value="nouveau" {{ old('status', $ticket->status) == 'nouveau' ? 'selected' : '' }}>Nouveau</option>
                        <option value="en_cours" {{ old('status', $ticket->status) == 'en_cours' ? 'selected' : '' }}>En_cours</option>
                        <option value="résolu" {{ old('status', $ticket->status) == 'résolu' ? 'selected' : '' }}>Résolu</option>
                        <option value="cloturé" {{ old('status', $ticket->status) == 'cloturé' ? 'selected' : '' }}>Cloturé</option>
                    </select>
                </div>
                    <!-- Pièce jointe -->
                    <div class="space-y-2">
    ...
    <script>
        ...
        form.addEventListener('submit', async function(e) {
            ...
            try {
                ...
                setTimeout(() => {
                    ...
                    window.location.href = "{{ route('chef.dashboard') }}";
                }, 3000);
        ...
        cancelBtn.addEventListener('click', function() {
            if (confirm('Êtes-vous sûr de vouloir annuler ? Toutes les modifications non enregistrées seront perdues.')) {
                window.location.href = "{{ route('chef.dashboard') }}"; // Redirect to dashboard on cancel
            }
        });
