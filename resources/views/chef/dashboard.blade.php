...
@section('title', 'Dashboard Chef - Tickets récents')
...
<div class="p-6 dark:bg-gray-900 min-h-screen">
            ...
            <table class="w-full">
                <thead>
                    <tr class="bg-gray-50 dark:bg-gray-700">
                        ...
                        <th class="py-4 px-6 text-left text-gray-700 dark:text-gray-200 font-semibold">Créé par</th>
                        <th class="py-4 px-6 text-left text-gray-700 dark:text-gray-200 font-semibold">Créé le</th>
                        <th class="py-4 px-6 text-left text-gray-700 dark:text-gray-200 font-semibold">Assigné à</th>
                        <th class="py-4 px-6 text-left text-gray-700 dark:text-gray-200 font-semibold">Actions</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse ($tickets as $ticket)
                        ...
                        <tr class="border-b border-gray-100 dark:border-gray-700">
                            ...
                            <td class="py-4 px-6 text-sm text-gray-600 dark:text-gray-300">
                                {{ $ticket->categorie->titre ?? 'Non défini' }}
                            </td>
                            ...
                            <td class="py-4 px-6 text-sm text-gray-600 dark:text-gray-300">
                                {{ optional($ticket->userSimple->user)->nom }} {{ optional($ticket->userSimple->user)->prenom }}
    ...
    @else
        <form method="POST" action="{{ route('chef.tickets.assign', $ticket->id) }}" class="flex items-center gap-2">
            @csrf
            <select
                name="technicien_id"
                required
                class="block w-full max-w-[90px] border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-800
                       text-gray-900 dark:text-gray-100 px-2 py-1.5 text-xs
            ...
            <button
                type="submit"
                class="inline-flex items-center justify-center w-7 h-7 rounded-md bg-indigo-600 hover:bg-indigo-700
                       focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-1 text-white transition"
                title="Confirmer l'assignation"
            >
                <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 stroke-2 stroke-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
